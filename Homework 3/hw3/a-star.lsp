...
;; type of pq is more efficient than a hep for problems where cost = depth.

(defun pq-create ()
  (let ((ans (make-array '(5001))))
...
(defun myht-create (size)
  (list size (make-array (list size))))
...
(let ((expanded 0)
  ...
  (defun reset () (setf expanded 0) (setf generated 1))
  ...
  (defun a*-search (open-list goal-p successors cost-fn remaining-cost-fn
                    closed-list)
    (loop
